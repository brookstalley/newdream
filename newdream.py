...
"""
Video dream alternate net method
...
def output_process(img,filename_suggested):
    if output_display:
        showarray(img)
    if output_save:
        savefile(img, filename_suggested)
...
def complex_dream(im, end_layer, recursion):
    frame = im[:]
    h, w = frame.shape[:2]
    s = 0.05  # scale coefficient
    for i in xrange(recursion):
...
input_file="lars-running-still.png"
output_file="lars-running-still-dream.png"
